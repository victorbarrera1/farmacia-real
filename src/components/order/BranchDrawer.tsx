...
/** Cajón selector de sucursal, sin movimiento. */
export function BranchDrawer() {
  ...
  return (
    <Drawer abierto={estado.cajon === 'suc'} onClose={() => dispatch({ type: 'cerrarCajones' })} titulo="Elige tu sucursal" subtitulo="El stock y el WhatsApp cambian según el local" labelId="tSuc" cerrarLabel="Cerrar selector">
      <div className="grid grid-cols-1 gap-3 py-2">
        {sucursales.map((s) => <BranchCard key={s.id} suc={s} elegida={s.id === estado.sucursal} onSelect={() => elegir(s.id, s.nombre)} />)}
      </div>
    </Drawer>
  );
}
